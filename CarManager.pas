...
procedure TFormCM.NiceGrid4DrawCell(Sender: TObject; ACanvas: TCanvas; X,
  Y: Integer; Rc: TRect; var Handled: Boolean);
begin
  if (NiceGrid4[04,y]='-') then
      ACanvas.Brush.Color := clGradientInactiveCaption;
    if (NiceGrid4[05,y]='-') then
      ACanvas.Brush.Color := clGradientInactiveCaption;
    if (NiceGrid4[06,y]='-') then
      ACanvas.Brush.Color := clGradientInactiveCaption;
    if (NiceGrid4[05,y]='') then ACanvas.Brush.Color := clYellow;
    if (NiceGrid4[06,y]='-') then ACanvas.Brush.Color := clYellow;
end;
...
var
  ...
  stmp  : Ansistring;
begin
  if (NiceGrid5[03,y]='') then ACanvas.Font.Color := clBlue; //현재미입금
  i := strtoint((NiceGrid5[06,y]));
  case i of
